...
根目錄的 `FitQuestApp.java` 是唯一正式入口，只負責將程式交給 `fithero.app.FitQuestApplication` 啟動。請不要再使用 `fithero.FitQuestApp`，該舊入口已移除。
...
└─ src/fithero/                        # 其他正式 Java 原始碼
   ├─ app/
   │  └─ FitQuestApplication.java      # 首次註冊、正式視窗啟動、提醒通知
   ├─ infra/
   │  └─ Storage.java                  # 玩家資料與歷史紀錄讀寫
   ├─ logic/
   │  ├─ calculator/ExpCalculator.java # 卡路里、強度與 XP 計算
   │  └─ manager/                      # 玩家狀態、升級、成就與健身目標
   ├─ model/
   │  ├─ achievement/                  # 成就資料模型
   │  ├─ exercise/                     # 運動項目、類型與肌群對應
   │  ├─ player/                       # 角色、個人資料與性別
   │  └─ workout/WorkoutEntry.java     # 單筆運動紀錄與 CSV 轉換
   └─ ui/
      ├─ FitQuestFrame.java            # 首頁 UI、頁面切換、最近紀錄、清空資料
      ├─ AvatarPanel.java              # 正反面角色合成與部位圖片對應
      ├─ WorkoutSessionDialog.java     # 新增運動與由清單添加項目
      ├─ WorkoutBarChartPanel.java     # 週／月／年趨勢與危險值
...
主要公式位於 `src/fithero/logic/calculator/ExpCalculator.java`，送出流程位於 `src/fithero/logic/manager/PlayerState.java`。
...
- 運動與肌群的對應定義於 `src/fithero/model/exercise/ExerciseRegistry.java`。
- 阻力訓練完成後，部位原始成長值為：
...
- 圖表實作：`src/fithero/ui/WorkoutBarChartPanel.java`。
...
| 想調整的項目 | 修改位置 |
|---|---|
| 全站主要 UI 顏色 | `src/fithero/ui/FitQuestFrame.java`、`WorkoutSessionDialog.java` 的頂部 `Color` 常數 |
| 角色合成畫布大小與留白 | `src/fithero/ui/AvatarPanel.java` 的 `CANVAS_SIZE`、`PREVIEW_MARGIN`、`COMPOSE_SIZE` |
...
- **修改運動項目**：編輯 `src/fithero/model/exercise/ExerciseRegistry.java`。新增項目時需設定名稱、肌群、有氧／阻力類型、自重狀態與難度，並確認 `WorkoutSessionDialog.updateExerciseList()` 能顯示該項目。
- **修改危險值**：編輯 `src/fithero/ui/FitQuestFrame.java` 的 `DAILY_TRAINING_DANGER_THRESHOLD`；危險提示顏色位於 `WorkoutBarChartPanel.DANGER_COLOR`。
- **修改角色圖片等級對應**：編輯 `src/fithero/ui/AvatarPanel.java` 的 `LevelMetaData`、`usesSharedFrontAsset()` 與 `readFrontPartWithFallback()`。
- **修改首頁刷新內容**：`WorkoutSessionDialog` 完成紀錄後會呼叫 `FitQuestFrame.saveAndRefresh()`；完整首頁更新集中在 `FitQuestFrame.refreshAll()`。
...
- 根目錄 `FitQuestApp.java` 必須維持為唯一 `main` 入口；應用啟動流程放在 `src/fithero/app`，不要再建立其他啟動類別。
- 修改 UI 時，盡量只改 `src/fithero/ui` 內的版面與繪製程式，不要順手更動 `PlayerState` 或 `ExpCalculator` 的資料計算邏輯。
...
- 不要提交 `.class` 或 `demo-out`；兩者已由 `.gitignore` 排除，Demo 前再重新編譯。
...
1. 使用根目錄入口啟動程式，進入「＋ 新增今日訓練」。
2. 勾選至少一個動作後按「完成紀錄」。
3. 回到首頁確認最新紀錄出現在「最近訓練紀錄」最上方。
4. 同時確認今日訓練量、今日任務、XP、角色等級、部位等級、小人偶圖片與趨勢圖都有更新。
5. 一次勾選多個動作，確認最近紀錄依時間由新到舊排列。

### 測試圖片、清單與危險值

1. 持續訓練同一肌群直到部位等級提升，確認正面或背面對應圖片立即改變。
2. 暫時更名一張非必要部位圖片，確認程式仍能使用鄰近等級或共用素材顯示；測試後將檔名還原。
3. 進入「由清單添加項目」，測試關鍵字、分類與確認添加。
4. 新增足以超過 `DAILY_TRAINING_DANGER_THRESHOLD` 的訓練量，確認趨勢圖顯示紅色危險值提示。
5. 按下清空資料，確認必須再次確認且畫面提示「此操作無法復原」；不要在需要保留資料時確認清空。
...
2. 確認從專案根目錄執行編譯指令。
3. 編譯時必須同時包含根目錄 `FitQuestApp.java` 與 `src` 內所有 Java 檔案。
4. 執行時使用 `java -cp demo-out FitQuestApp`；不要再使用舊的 `fithero.FitQuestApp`。
