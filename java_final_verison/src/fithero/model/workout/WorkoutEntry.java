...
public class WorkoutEntry {
    ...
    /**
     * 首頁任務與趨勢圖共用的訓練量，不改變原始紀錄或 CSV 格式。
     * 阻力訓練使用重量 × 次數 × 組數；自重與有氧以完成量計入。
     */
    public double trainingVolume() {
        ExerciseInfo info = ExerciseRegistry.getExercise(exerciseName);
        if (info != null && info.isAerobic()) return amount * sets;
        return (weight > 0.0 ? weight : 1.0) * amount * sets;
    }
